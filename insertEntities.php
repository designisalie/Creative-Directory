<?php
	...
	if($type == 'hott'){
		$query = mysql_query("SELECT * FROM entities ORDER BY rank DESC LIMIT $more,10");
	}elseif($type == 'new'){	
		$query = mysql_query("SELECT * FROM entities ORDER BY id DESC LIMIT $more,10");
	}else{	
		$query = mysql_query("SELECT * FROM entities WHERE type='$type' ORDER BY rank DESC LIMIT $more,10");
	}
	...
	$count = $more + 10;
	if($type == 'hott' || $type == 'new'){
		$check  = mysql_query("SELECT * FROM entities LIMIT $count,1");
	}else{	
		$check  = mysql_query("SELECT * FROM entities WHERE type='$type' LIMIT $count,1");
	}
	$quantity = mysql_num_rows($check);
	if($quantity > 0){
		echo "<button onclick='Insert(this)' class='more' name='" . $count . "'>more.</button>";
	}
